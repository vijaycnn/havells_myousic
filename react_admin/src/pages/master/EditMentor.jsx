...
function EditMentor() {
    ...
    const getMentor = async () => {
        ...
        await axiosInstance.get(`/mentor/getById/${id}`)
          .then((response) => {
    ...
    const validation = (values) => {
        setError('');        
        let hasError = false;
        if (!values.name || values.name == "" || (uploadMediaFile == null && previousData.fileUrl == '')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (!hasError && previousData.id > 0) {
            ...
            await axiosInstance.post(`/mentor/valid`, validateBody)
  ...
  const getUploadUrl = async (file) => {
    ...
    const formProcess = async (fileUrl) => {        
        ...
        await axiosInstance.post(`/mentor/update`, body)
            .then((response) => {
        ...
        <Form onSubmit={handleSubmit} className="login-form p-xl-0 p-md-5 p-4 col-xl-12 m-auto">
            ...
            <Row>                
                <Col md={4}>
                    <Form.Group className="mb-4">
                        ...
                        <Form.Control type="text" name="name" value={data.name} placeholder="Enter Name"
                ...
                {/* image preview */}
                {
                    (data.fileUrl != '') ?
                    <>
                    <img src={data.fileUrl} width={150} height={150} />
                    </>: ''
                }
                </Col>
            </Row>
            ...
            <Col md={4}>
                <Form.Group className="mb-4">
                    ...
                    <Form.Control type="text" name="title" value={data.title} placeholder="Enter Title"
                        onChange={handleChange}  />  
                </Form.Group>
            </Col>
            <Col md={9}>
