...
function Banner() {
  ...
  const getBanners = async () => {
    ...
    await axiosInstance.get(`/banner/list`, body)
			.then((response) => {
  ...
  const changeStatus = async(index, currentStatus, categoryId)=>{
    ...
    // console.log('body>>> ', body);
    await axiosInstance.post(`/banner/changeStatus`, body)
			.then((response) => {
        // console.log('>>> ', response.data);
				setIsLoading(false)
				if (response.data.status === "success") {
                ...
                <>
                  <tr key={item.id}>
                    ...
                    <td className="col-fixed">
                        <Link title="Edit" to={`${adminAlias}/editBanner/${base64_encode(`Hvg_myg8Bbg5vvdgvpp+`+item.id)}`} className="btn btn-icon">
                            <BiPencil />
                        </Link>
                        &nbsp;
  ...
  return (
    <>
      <h1 className="h4 mb-4 font-secondary fw-medium">Banner</h1>
      ...
      <div className="table-view bg-white rounded-4 p-4">
        <div className="mb-3 d-flex justify-content-between align-items-center">
          ...
          <div>
              <Link to={`${adminAlias}/addBanner`} className="btn btn-primary btn-sm">
                  <span className="nav-link-text">Add Banner-Image</span>
              </Link>              
          </div>
        </div>
        {
