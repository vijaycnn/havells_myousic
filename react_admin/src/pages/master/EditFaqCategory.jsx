...
function EditFaqCategory() {
    ...
    const getFaqCategory = async () => {
        ...
        await axiosInstance.get(`/faq/getCategoryById/${id}`)
                .then((response) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        if (!data.name || data.name.trim() == '') {
            setError('Missing Required!'); return;
        }
        setIsSubmit(true);    
        setLoading(true);
        let body = {
        ...
        await axiosInstance.post(`/faq/updateCategory`, body)
            .then((response) => {
                  // console.log('>>> ', response.data);
                if (response.data.status === "success") {
                    setData({ name: ""});                    
  ...
  return (
    <>
      ...
      <div className="mb-3 d-flex justify-content-between align-items-center">
        ...
        <div>
            <Link to={`${adminAlias}/category`} className="btn btn-primary btn-sm">
